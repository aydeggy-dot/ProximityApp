...
export const useProximityDetection = (
  ...
  const prevNearbyUserIds = useRef<Set<string>>(new Set());
  ...
  const triggerProximityAlert = useCallback(
    ...
    [user?.uid, userPreferences, groupId, recentAlerts, debounceWindowMs, onProximityAlert]
  );
  ...
  const checkProximity = useCallback(
    async (location: Location, users: UserLocation[]) => {
      ...
      // Check for new nearby users and trigger alerts
      const currentNearbyIds = new Set(nearbyWithProfiles.map(u => u.userId));
      const newNearbyUsers = nearbyWithProfiles.filter(
        u => !prevNearbyUserIds.current.has(u.userId)
      );

      // Trigger alerts for new nearby users
      for (const nearbyUser of newNearbyUsers) {
        await triggerProximityAlert(nearbyUser, nearbyUser.distance);
      }

      // Update previous nearby users
      prevNearbyUserIds.current = currentNearbyIds;
    },
    [user?.uid, userPreferences, radiusMeters, triggerProximityAlert]
  );

  /**
   * Subscribe to nearby users in the group
   */
  useEffect(() => {
    ...
    console.log(`Starting proximity detection for group ${groupId}`);

    const unsubscribe = subscribeToNearbyUsers(groupId, (locations) => {
      setAllUsers(locations);

      // If we have current location, check proximity
      if (currentLocation && isTracking) {
        checkProximity(currentLocation, locations);
      }
    });

    return () => {
      console.log(`Stopping proximity detection for group ${groupId}`);
      unsubscribe();
      setIsDetecting(false);
    };
  }, [enabled, groupId, user?.uid, currentLocation, isTracking, checkProximity]);
  ...
  const clearAlerts = useCallback(() => {
    setRecentAlerts([]);
    prevNearbyUserIds.current.clear();
  }, []);
