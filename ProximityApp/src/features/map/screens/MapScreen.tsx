...
import { GroupMembership, ProximityAlert as ProximityAlertType, MapStackParamList } from '../../../types';
...
import MemberInfoSheet from '../../../components/MemberInfoSheet';
...
const MapScreen: React.FC = () => {
  ...
  // Always enabled when group is selected (to see other users)
  // But alerts only trigger when broadcasting
  const proximityDetection = useProximityDetection({
    groupId: selectedGroupId || '',
    enabled: !!selectedGroupId,
    triggerAlertsOnlyWhenBroadcasting: isBroadcasting,
  ...
  const toggleBroadcasting = async () => {
    ...
    try {
      ...
      const updatePromise = updateGroupMembership(user.uid, selectedGroupId, {
      ...
      // Immediately sync location if broadcasting was enabled
      if (newBroadcastingState && currentLocation) {
        console.log('Broadcasting enabled, syncing location immediately...');
        // Trigger immediate location sync
        setTimeout(() => {
          locationSync.syncNow();
        }, 1000); // Small delay to ensure Firestore write completes
      }
      ...
      console.error('[MapScreen] Error toggling broadcasting:', {
      ...
      Alert.alert(
        ...
        isTimeout
          ...
          : `Failed to update broadcasting status: ${errorMessage}`
      );

      // Reload the correct state from Firestore
      loadBroadcastingStatus(selectedGroupId);
    } finally {
  ...
  /**
   * Handle marker press - show member info sheet
   */
  const handleMarkerPress = async (nearbyUser: any) => {
    try {
      // Fetch full user profile
      const profile = await getUserProfile(nearbyUser.userId);

      setSelectedMember({
        userId: nearbyUser.userId,
        displayName: nearbyUser.userProfile?.displayName || 'Group Member',
      ...
      // Show sheet anyway with basic info
      setSelectedMember({
        userId: nearbyUser.userId,
        displayName: nearbyUser.userProfile?.displayName || 'Group Member',
        photoURL: nearbyUser.userProfile?.photoURL,
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Generate chat ID
      const chatId = generateChatId(user.uid, selectedMember.userId);
      ...
      navigation.navigate('ChatScreen', {
        ...
        otherUserName: selectedMember.displayName,
        otherUserPhoto: selectedMember.profilePicture || selectedMember.photoURL,
      });
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {/* Nearby users markers - show all broadcasting users regardless of own status */}
      ...
      {selectedMember && (
        <MemberInfoSheet
          ...
          onClose={() => setMemberInfoVisible(false)}
