...
type TabType = 'my-groups' | 'nearby' | 'discover' | 'created';
...
const GroupsListScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const { groups: nearbyGroups, loading: nearbyLoading, error: nearbyError, hasMore: nearbyHasMore, loadMore: loadMoreNearby, refresh: refreshNearby } = useNearbyGroups(20);
  const { groups: popularGroups, loading: popularLoading, error: popularError, hasMore: popularHasMore, loadMore: loadMorePopular, refresh: refreshPopular } = usePopularGroups(20);
  ...
  // Show errors
  useEffect(() => {
    const error = myGroupsError || searchError || nearbyError || popularError;
    if (error) {
  ...
  }, [myGroupsError, searchError, nearbyError, popularError]);
  ...
  const getFilteredGroups = () => {
    if (activeTab === 'my-groups') {
      ...
      // 'discover' tab shows popular groups
  ...
  const renderSearchBar = () => {
    // Show search bar for both nearby and discover tabs
    if (activeTab !== 'nearby' && activeTab !== 'discover') return null;
    ...
    return (
      <View style={[styles.searchContainer, { backgroundColor: theme.colors.surface }]}>
        ...
        <TextInput
          ...
          placeholder={activeTab === 'nearby' ? 'Search nearby groups...' : 'Search popular groups...'}
  ...
  const renderInviteCodeInput = () => {
    // Only show invite code input on discover tab
  ...
  const renderEmptyState = () => {
    if (activeTab === 'nearby') {
      return (
        <View style={styles.emptyContainer}>
          <Icon name="map-marker-radius" size={64} color={theme.colors.textTertiary} />
          <Text style={[styles.emptyTitle, { color: theme.colors.text }]}>
            No Nearby Groups
          </Text>
          <Text style={[styles.emptySubtitle, { color: theme.colors.textSecondary }]}>
            There are no public groups near your location
          </Text>
        </View>
      );
    }
    ...
    if (activeTab === 'discover') {
      return (
        <View style={styles.emptyContainer}>
          <Icon name="fire" size={64} color={theme.colors.textTertiary} />
          <Text style={[styles.emptyTitle, { color: theme.colors.text }]}>
            {searchQuery ? 'No Groups Found' : 'No Popular Groups'}
          </Text>
          <Text style={[styles.emptySubtitle, { color: theme.colors.textSecondary }]}>
            {searchQuery
              ? 'Try a different search term'
              : 'Check back later for popular groups'}
          </Text>
        </View>
      );
    }
  ...
  const renderGroupItem = ({ item }: { item: Group }) => {
    // Check if user actually has this group in their "My Groups"
    const isInMyGroups = myGroups.some(g => g.id === item.id);
    // Show join button if not in "My Groups" for both nearby and discover tabs
    const showJoinButton = (activeTab === 'discover' || activeTab === 'nearby') && !isInMyGroups;
    ...
    return (
      <View>
        <GroupCard
          ...
          showDistance={activeTab === 'nearby' && item.distance !== undefined}
        />
        {showJoinButton && (
  ...
  const isLoading = (() => {
    if (activeTab === 'my-groups' || activeTab === 'created') return myGroupsLoading;
    if (activeTab === 'nearby') return nearbyLoading;
    if (activeTab === 'discover') return popularLoading;
  ...
  const hasMore = (() => {
    if (activeTab === 'nearby') return nearbyHasMore;
    if (activeTab === 'discover') return popularHasMore;
    return false;
  })();
  ...
  const handleRefresh = () => {
    if (activeTab === 'my-groups' || activeTab === 'created') {
      refreshMyGroups();
    } else if (activeTab === 'nearby') {
      refreshNearby();
    } else if (activeTab === 'discover') {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={[styles.tabsContainer, { backgroundColor: theme.colors.background }]}>
        ...
        {renderTabButton('nearby', 'Nearby', 'map-marker-radius')}
        {renderTabButton('discover', 'Popular', 'fire')}
