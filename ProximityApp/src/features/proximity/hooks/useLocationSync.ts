...
export const useLocationSync = (options: UseLocationSyncOptions = {}): UseLocationSyncResult => {
  ...
  const fetchBroadcastingGroups = useCallback(async () => {
    if (!user?.uid) {
      console.log('[LocationSync] No user, clearing broadcasting groups');
    ...
    try {
      console.log(`[LocationSync] Fetching memberships for user ${user.uid}...`);
      const memberships = await getGroupMemberships(user.uid);
      console.log(`[LocationSync] Found ${memberships.length} total memberships`);
      const broadcasting = memberships.filter(m => m.isBroadcasting);
      console.log(`[LocationSync] Found ${broadcasting.length} broadcasting memberships:`,
        broadcasting.map(m => `Group ${m.groupId}: ${m.isBroadcasting}`));
      broadcatingMemberships.current = broadcasting;
      setActiveGroupCount(broadcasting.length);
    } catch (err) {
      console.error('[LocationSync] Error fetching broadcasting groups:', err);
  ...
  const syncLocation = useCallback(async (location: Location, force = false) => {
    ...
    // ALWAYS fetch fresh broadcasting groups to catch any recent changes
    await fetchBroadcastingGroups();

    const groups = broadcatingMemberships.current;
    if (groups.length === 0) {
      // No broadcasting groups, nothing to sync
      console.log('No broadcasting groups found, skipping location sync');
      return;
    }

    console.log(`Found ${groups.length} broadcasting group(s), syncing location...`);
    ...
    try {
      ...
      console.log(`[LocationSync] Syncing location (${location.latitude.toFixed(6)}, ${location.longitude.toFixed(6)}) to ${groups.length} group(s)...`);
      const syncPromises = groups.map(membership => {
        console.log(`[LocationSync] - Syncing to group ${membership.groupId}`);
        return updateUserLocation(user.uid, membership.groupId, location);
      });
      ...
      console.log(`[LocationSync] ✓ Location synced successfully to ${groups.length} group(s) at ${new Date(now).toLocaleTimeString()}`);
    } catch (err) {
