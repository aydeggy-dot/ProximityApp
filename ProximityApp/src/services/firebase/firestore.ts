...
export const addGroupMember = async (
  ...
  // Increment member count - don't fail the whole operation if this fails
  try {
    await firestore()
    ...
    console.warn('Failed to increment member count (non-critical):', err);
...
export const updateGroupMembership = async (
  ...
  const membershipId = `${userId}_${groupId}`;
  console.log(`[Firestore] Updating membership ${membershipId} with:`, updates);

  try {
    // First check if the membership exists
    const existingMembership = await getDocument<GroupMembership>(
      COLLECTIONS.GROUP_MEMBERSHIPS,
      membershipId
    );

    if (!existingMembership) {
      const error = new Error(`Membership document ${membershipId} does not exist`);
      console.error('[Firestore] Membership not found:', error);
      throw error;
    }

    console.log(`[Firestore] Existing membership:`, existingMembership);

    // Use updateDocument for existing documents (better for security rules)
    await updateDocument(COLLECTIONS.GROUP_MEMBERSHIPS, membershipId, updates);
    console.log(`[Firestore] ✓ Membership updated successfully`);
  } catch (error) {
    console.error('[Firestore] Error updating membership:', {
      membershipId,
      updates,
...
/**
 * Get public groups with pagination support
 * @param limit - Number of groups to fetch per page (default: 20)
 * @param lastVisible - Last document from previous page (for cursor-based pagination)
 * @returns Object containing groups array and last visible document for next page
 */
export const getPublicGroupsPaginated = async (
  limit: number = 20,
  lastVisible?: FirebaseFirestoreTypes.DocumentSnapshot
): Promise<{ groups: Group[]; lastVisible: FirebaseFirestoreTypes.DocumentSnapshot | null }> => {
  try {
    let query = firestore()
      .collection(COLLECTIONS.GROUPS)
      .where('isActive', '==', true)
    ...
    return { groups, lastVisible: lastDoc };
  } catch (error) {
    console.error('Error getting paginated public groups:', error);
    throw error;
  }
};

/**
 * Get popular groups sorted by popularity score with pagination
 * @param limit - Number of groups to fetch per page (default: 20)
 * @param lastVisible - Last document from previous page
 * @returns Object containing groups array and last visible document for next page
 */
...
/**
 * Search groups with pagination support
 * @param searchQuery - Search term for group name/description
 * @param limit - Number of groups to fetch per page (default: 20)
 * @param lastVisible - Last document from previous page
 * @returns Object containing groups array and last visible document for next page
 */
export const searchGroupsPaginated = async (
  searchQuery: string,
  limit: number = 20,
  lastVisible?: FirebaseFirestoreTypes.DocumentSnapshot
): Promise<{ groups: Group[]; lastVisible: FirebaseFirestoreTypes.DocumentSnapshot | null }> => {
  try {
    // For basic search, we'll fetch groups and filter client-side
    // In production, consider using Algolia for full-text search
    let query = firestore()
      .collection(COLLECTIONS.GROUPS)
      .where('isActive', '==', true)
      .where('privacyLevel', 'in', ['public', 'invite-only'])
    ...
    const allGroups = snapshot.docs.map(doc => {
      const data = convertTimestamps(doc.data());
      return { id: doc.id, ...data, _doc: doc } as Group & { _doc: FirebaseFirestoreTypes.DocumentSnapshot };
    });
    ...
    const groups = filteredGroups.slice(0, limit).map(g => {
      const { _doc, ...group } = g;
      return group as Group;
    });

    const lastDoc = filteredGroups.length > 0
      ? filteredGroups[filteredGroups.length - 1]._doc
      : null;

    return { groups, lastVisible: lastDoc };
  } catch (error) {
    console.error('Error searching groups with pagination:', error);
    throw error;
  }
};
